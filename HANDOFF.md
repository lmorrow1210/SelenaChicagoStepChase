...
- **Game engines (M4/M5/M6 core logic, pure functions, fully unit-tested — 25 api tests green total):**
  - `src/services/prediction.ts` — `scorePredictions` (min delta wins, tie → earliest submitted_at, exactly one winner).
  - `src/services/bingo.ts` — `countBingoLines` (5+5+2 lines, free space at 12), `isBlackout`, `generateCard` (24 distinct + free space, category-balanced round-robin, injectable RNG), `evaluateDetector` (handles steps/workouts/azm/sleep/hr_zones/workout_before/streaks/group totals/rank/hot-pursuit/rest-day/weekday; un-evaluable metrics like `steps_before`/`bedtime_before` return false = tile stays incomplete — wire intraday/multi-night data later).
  - `src/services/nemesis.ts` — `decideDay` (steps → target_hit_at → tie), `tallyScore`, `weekOutcome` ('tiebreak' = Saturday sudden death), `pairPlayers` (odd → bye).
  - These are NOT yet wired to routes/DB — the remaining M4–M6 work is the persistence + screen layer around these functions, plus the route endpoints in plan §3.
