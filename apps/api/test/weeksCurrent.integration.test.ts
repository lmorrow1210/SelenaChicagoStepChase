...
describeDb("current week route integration", () => {
  ...
  it("extends current Chicago week with authoritative Week 1 season state", async () => {
    ...
    const phase = calculateWeeklyPhase({
      ...
      finalizedAt: null,
      dataConfidence: "incomplete",
      briefingViewed: body.seasonState.ritualViews.mondayBriefing,
      midweekViewed: body.seasonState.ritualViews.midweekUpdate,
      finalPushViewed: body.seasonState.ritualViews.finalPush,
      caseClosedViewed: body.seasonState.ritualViews.caseClosed,
